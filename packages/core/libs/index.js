...
function registerCommand() {
  ...
  program
    ...
    .action(async (type, { packagePath, force }) => {
      const packageName = '@wuan-cli/init';
  ...
  program
    ...
    .action(async ({
      ...
      const packageName = '@wuan-cli/publish';
      const packageVersion = '1.0.0';
      if (force) {
        refreshToken = true;
        refreshOwner = true;
  ...
  program
    ...
    .action(async ({ packagePath, region, bucket, ossAccessKey, ossSecretKey }) => {
      const packageName = '@wuan-cli/replace';
      const packageVersion = '1.0.0';
...
function checkNodeVersion() {
  ...
  if (!semver.gte(process.version, LOWEST_NODE_VERSION)) {
    throw new Error(colors.red(`wuan-cli 需要安装 v${LOWEST_NODE_VERSION} 以上版本的 Node.js`));
  }
}
